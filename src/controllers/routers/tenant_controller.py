...
router = RouterConfig.get_unprotected_router()

@cbv(router)
class TenantController:

    @router.post('/tokens/', status_code=status.HTTP_201_CREATED, response_model=TenantTokenReadDTO)
    async def register_tenant(self) -> TenantTokenReadDTO:
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="failed generating tokens")
        return dto

    @router.get('/tokens/{token_id}', status_code=status.HTTP_200_OK, response_model=TenantTokenReadDTO)
